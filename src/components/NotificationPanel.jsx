    ...
    <>
      <Popover className="relative">
        <Popover.Button className="inline-flex items-center outline-none">
          <div className="w-8 h-8 flex items-center justify-center text-gray-800 relative mt-[2px]">
            {/* <IoIosNotificationsOutline className="text-3xl text-gray-800" /> */}

            <IoIosNotifications className="text-3xl text-yellow-500 dark:text-white" />

            {data?.length > 0 && (
              <span className="absolute text-center top-0 right-1 text-sm text-white font-semibold w-[18px] h-[18px] rounded-full bg-red-600">
                {data?.length}
              </span>
            )}
          </div>
        ...
        >
          <Popover.Panel className="absolute -right-10 md:-right-2 z-10 mt-5 flex w-screen max-w-max px-4">
            {({ close }) =>
              data?.length > 0 && (
                <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white dark:bg-[#3f3f3f] text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                  <div className="p-4">
                    {data?.slice(0, 5).map((item, index) => (
                      <div
                        key={item._id + index}
                        className="group relative flex gap-x-4 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-white/10"
                      >
                  ...
                  <div className="grid grid-cols-2 divide-x bg-gray-50 dark:bg-[#282828]">
                    {callsToAction.map((item) => (
                      <Link
                        ...
                        className="flex items-center justify-center gap-x-2.5 p-3 font-semibold text-blue-600 dark:text-white hover:bg-gray-100 dark:hover:bg-white/20"
                      >
                        {item.name}
                      </Link>
                    ))}
